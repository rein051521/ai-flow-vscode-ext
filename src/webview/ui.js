...
function renderHtml(state) {
    ...
    const evidenceList = state.evidence.length
        ...
        : "(none)";
    const tabButton = (id, label, active) => `<button class="tabbtn${active ? " active" : ""}" data-tab="${id}">${escapeHtml(label)}</button>`;
    const tabPanel = (id, content, active) => `<div class="tabpanel${active ? " active" : ""}" id="tab-${id}"><pre>${escapeHtml(content)}</pre></div>`;
    return `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8" />
